...
- **领域工具（348 个）**：通过 `call_tool` 代理，由 `register_all.cpp` 中的 `g_handlers` 映射分发。完整 schema 在 `ToolCatalog` 中
- **工具总数**：355 = 7 元 + 348 领域；`ToolCatalog` 条目 356（另含 `system_status` 与 3 个 meta 快照）
...
- **结构**：L1 = `gsd_unit_tests`（59 个 gtest，不启动引擎，含 355 工具注册管线断言）；L2 = `gsd_test_runner` + `tests/config/*.json`（5 个用例文件，每文件一次 headless 编辑器生命周期最小闭环，经真实 MCP HTTP）
...
- **全工具遍历**：`03_tools_contract.json` 对 348 领域工具做空参契约 + 启发式冒烟（560 步，约 15s）
...
- **无 CI**（`.github/` 不存在）
- **schema 覆盖**：283 非空 / 73 空（`tests/unit/register_all_test.cpp:26-27` 的 `SchemaStatisticsBaseline` 硬断言锁定）；**3 个契约缺口**：`scene_node_create`（name/type 有默认值不校验必填）、`resource_get_extensions`（缺 type 返回全类型）、`resource_reimport`（空参 count=0 静默成功）——遍历测试记 warnings 不 FAIL
