...
static const char* LEVEL_ICON_NAMES[4] = {
    "Debug", "Popup", "StatusWarning", "StatusError"
};

static const char* LEVEL_TOOLTIPS[4] = {
...
McpLogDock::McpLogDock()
    ...
    collapse_button->set_tooltip_text("Collapse consecutive identical messages into one entry. Shows number of occurrences.");
    ...
    search_box->connect("text_changed", callable_mp(this, &McpLogDock::_on_search_changed));
    for (int i = 0; i < 4; i++) {
        filter_buttons[i]->connect("toggled", callable_mp(this, &McpLogDock::_on_filter_toggled).bind(i));
    }
    category_filter->connect("item_selected", callable_mp(this, &McpLogDock::_on_category_changed));
    clear_button->connect("pressed", callable_mp(this, &McpLogDock::_on_clear));
    collapse_button->connect("toggled", callable_mp(this, &McpLogDock::_on_collapse_toggled));
...
void McpLogDock::_update_theme() {
    ...
    theme_cache.warning_color = theme->has_color("warning_color", editor_type)
    ...
    if (theme->has_font("output_source", editor_fonts))
        log_display->add_theme_font_override("normal_font", theme->get_font("output_source", editor_fonts));
    if (theme->has_font("output_source_bold", editor_fonts))
        log_display->add_theme_font_override("bold_font", theme->get_font("output_source_bold", editor_fonts));
    if (theme->has_font("output_source_italic", editor_fonts))
        log_display->add_theme_font_override("italics_font", theme->get_font("output_source_italic", editor_fonts));
    if (theme->has_font("output_source_mono", editor_fonts))
        log_display->add_theme_font_override("mono_font", theme->get_font("output_source_mono", editor_fonts));
    ...
    if (theme->has_icon("CombineLines", editor_icons))
        collapse_button->set_button_icon(theme->get_icon("CombineLines", editor_icons));
    ...
    for (int i = 0; i < 4; i++) {
        if (theme->has_icon(LEVEL_ICON_NAMES[i], editor_icons))
            filter_buttons[i]->set_button_icon(theme->get_icon(LEVEL_ICON_NAMES[i], editor_icons));
    }
}

void McpLogDock::refresh() {
    _rebuild_log();
}

void McpLogDock::poll_new_entries() {
    auto entries = log_system->query({});
    if (entries.size() <= last_shown_count_) return;
...
bool McpLogDock::_add_log_line(const LogEntry& entry, int count) {
    if (!_check_display(entry)) return false;

    _apply_entry_style(entry.level);
    if (collapse && count > 1) {
        log_display->add_text("(" + godot::String::num_int64(count) + ")" + entry.message.c_str());
    } else {
...
bool McpLogDock::_check_display(const LogEntry& entry) const {
    ...
    if (!filter_buttons[level_idx]->is_pressed()) return false;

    if (category_filter->get_selected() > 0) {
        LogCategory selected_cat = static_cast<LogCategory>(category_filter->get_selected() - 1);
        if (entry.category != selected_cat) return false;
    }

    if (!search_box->get_text().is_empty()) {
        godot::String filter = search_box->get_text();
        godot::String msg = entry.message.c_str();
        if (msg.findn(filter) == -1) return false;
    }
...
void McpLogDock::_rebuild_log() {
    ...
    // collapse ON: segment-based grouping
...

    {
        ...
        for (const auto* e : entries) {
            ...
            if (freq[e->message] == 1) {
                ...
                auto le = LogEntry{{}, msg_level[e->message], msg_cat[e->message], e->message};
                _add_log_line(le, 1);
                continue;
            }
