...
public:
  void _log_error(const godot::String &p_function, const godot::String &p_file,
    ...
    push_game_error(to_std(p_file), to_std(p_function), p_line, to_std(p_code),
                    to_std(p_rationale),
...
godot::Dictionary find_property_info(godot::Node *node,
  ...
  for (int64_t i = 0; i < props.size(); i++) {
    ...
    if (dict.has("name") &&
        to_std(dict["name"].operator godot::String()) == prop_name) {
      return dict;
    }
  }
...
JV op_eval_set_property(const JV &params) {
  ...
  if (prop_info.has("type")) {
    ...
    if ((is_object_type || is_resource_hint) && prop_info.has("hint_string")) {
      std::string hint_str =
          to_std(prop_info["hint_string"].operator godot::String());
      if (!hint_str.empty()) {
        type_hint = hint_str;
      }
    }
