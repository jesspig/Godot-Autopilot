...
godot::Key parse_key(const std::string& key_str) {
    if (key_str.empty()) return godot::KEY_NONE;
    ...
    if (u == "SPACE") return godot::KEY_SPACE;
    if (u == "ENTER") return godot::KEY_ENTER;
    if (u == "ESCAPE") return godot::KEY_ESCAPE;
    if (u == "SHIFT") return godot::KEY_SHIFT;
    if (u == "CONTROL" || u == "CTRL") return godot::KEY_CTRL;
    if (u == "ALT") return godot::KEY_ALT;
    if (u == "TAB") return godot::KEY_TAB;
    if (u == "BACKSPACE") return godot::KEY_BACKSPACE;
    if (u == "DELETE") return godot::KEY_DELETE;
    if (u == "LEFT") return godot::KEY_LEFT;
    if (u == "RIGHT") return godot::KEY_RIGHT;
    if (u == "UP") return godot::KEY_UP;
    if (u == "DOWN") return godot::KEY_DOWN;
...
godot::MouseButton parse_mouse_button(const std::string& button_str) {
    ...
    if (u == "LEFT") return godot::MOUSE_BUTTON_LEFT;
    if (u == "RIGHT") return godot::MOUSE_BUTTON_RIGHT;
    if (u == "MIDDLE") return godot::MOUSE_BUTTON_MIDDLE;
...
mcp::JsonValue handle_is_action_just_pressed(const mcp::JsonValue& args) {
    ...
    std::string action_str = action_p->GetString();
    bool result = input->is_action_just_pressed(godot::StringName(action_str.c_str()));
...
mcp::JsonValue handle_gamepad_simulate(const mcp::JsonValue& args) {
    ...
    if (u == "STOP") {
        ...
        if (dev_p && dev_p->IsInt()) device = dev_p->GetInt();
