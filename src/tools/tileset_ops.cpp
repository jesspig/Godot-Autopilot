...
godot::Ref<godot::TileSet> resolve_tileset(const std::string& name, JV& out_error) {
    godot::Ref<godot::Resource> res = resource_ops::resolve_memory_resource(name);
    if (res.is_null()) {
        out_error = error("memory resource not found: " + name + " (create it with tileset_create first)");
...
bool parse_vec2i(const JV& obj, const char* x_key, const char* y_key, godot::Vector2i& out) {
...
JV handle_add_atlas_source(const JV& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "tileset_add_atlas_source called");

    auto* n = args.Find("name");
    if (!n || !n->IsString()) return error("missing required parameter: name");
    auto* sid = args.Find("source_id");
    if (!sid || !sid->IsInt()) return error("missing required parameter: source_id");
    auto* tex = args.Find("texture");
    if (!tex || !tex->IsString()) return error("missing required parameter: texture");
    auto* tsz = args.Find("tile_size");
    if (!tsz || !tsz->IsObject()) return error("missing required parameter: tile_size");
    godot::Vector2i tile_size;
    if (!parse_vec2i(*tsz, "x", "y", tile_size)) return error("missing required parameter: tile_size.x");

    int margin = 0;
    auto* mg = args.Find("margin");
    if (mg && mg->IsInt()) margin = static_cast<int>(mg->GetInt());

    int spacing = 0;
    auto* sp = args.Find("spacing");
    if (sp && sp->IsInt()) spacing = static_cast<int>(sp->GetInt());

    JV resolve_err;
    godot::Ref<godot::TileSet> tileset = resolve_tileset(n->GetString(), resolve_err);
    if (tileset.is_null()) return resolve_err;
    ...
    godot::Variant var = cdbs->instantiate(godot::StringName("TileSetAtlasSource"));
    if (var.get_type() == godot::Variant::NIL) return error("failed to instantiate TileSetAtlasSource");
    godot::Ref<godot::TileSetAtlasSource> src = var;
    if (src.is_null()) return error("instantiated object is not a TileSetAtlasSource");
...
JV handle_add_physics_layer(const JV& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "tileset_add_physics_layer called");

    auto* n = args.Find("name");
    if (!n || !n->IsString()) return error("missing required parameter: name");
    auto* lid = args.Find("layer_id");
    if (!lid || !lid->IsInt()) return error("missing required parameter: layer_id");

    int collision_layer = 1;
    auto* cl = args.Find("collision_layer");
    if (cl && cl->IsInt()) collision_layer = static_cast<int>(cl->GetInt());

    int collision_mask = 1;
    auto* cm = args.Find("collision_mask");
    if (cm && cm->IsInt()) collision_mask = static_cast<int>(cm->GetInt());
    ...
    tileset->set_physics_layer_collision_layer(actual_id, static_cast<uint32_t>(collision_layer));
    tileset->set_physics_layer_collision_mask(actual_id, static_cast<uint32_t>(collision_mask));

    JV r(JV::object_tag);
    r["result"] = JV("ok");
    r["layer_id"] = JV(static_cast<int64_t>(actual_id));
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "tileset_add_physics_layer completed");
    return r;
}

JV handle_set_tile_collision(const JV& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "tileset_set_tile_collision called");
    ...
    if (!ac || !ac->IsObject()) return error("missing required parameter: atlas_coords");
    godot::Vector2i coords;
    if (!parse_vec2i(*ac, "x", "y", coords)) return error("missing required parameter: atlas_coords.x");
    auto* pl = args.Find("physics_layer");
    if (!pl || !pl->IsInt()) return error("missing required parameter: physics_layer");
    auto* poly = args.Find("polygon");
    if (!poly || !poly->IsArray()) return error("missing required parameter: polygon");
    ...
    if (src_res.is_null()) return error("source not found: " + std::to_string(source_id));
    godot::Ref<godot::TileSetAtlasSource> src = src_res;
    if (src.is_null()) return error("source " + std::to_string(source_id) + " is not a TileSetAtlasSource");
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "tileset_set_tile_collision completed");
    return r;
}
