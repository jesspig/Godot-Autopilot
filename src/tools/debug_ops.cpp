...
mcp::JsonValue handle_print(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_print called");
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_print completed");
    return r;
}

mcp::JsonValue handle_print_stack(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_print_stack called");
    ...
    for (int i = 0; i < traces.size(); i++) {
        ...
        for (int f = 0; f < frame_count; f++) {
            ...
            frame["line"] = mcp::JsonValue(static_cast<int64_t>(bt->get_frame_line(f)));
        ...
        if (include_vars) {
            ...
            for (int f = 0; f < frame_count; f++) {
                ...
                for (int l = 0; l < lvc; l++) {
                    ...
                    lv["name"] = mcp::JsonValue(to_std(bt->get_local_variable_name(f, l)));
            ...
            for (int f = 0; f < frame_count; f++) {
                ...
                for (int m = 0; m < mvc; m++) {
                    ...
                    mv["name"] = mcp::JsonValue(to_std(bt->get_member_variable_name(f, m)));
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_print_stack completed");
    return r;
}

mcp::JsonValue handle_get_performance_monitor(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_get_performance_monitor called");
    auto* mp = args.Find("monitor");
    if (!mp || !mp->IsNumber()) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("missing required parameter: monitor (integer 0-58)");
        return e;
    }
    int64_t monitor_id = mp->IsDouble() ? static_cast<int64_t>(mp->GetDouble()) : mp->GetInt();
    if (monitor_id < 0 || monitor_id >= static_cast<int64_t>(godot::Performance::MONITOR_MAX)) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("monitor id out of range: " + std::to_string(monitor_id));
        return e;
    }
    ...
    double value = perf->get_monitor(static_cast<godot::Performance::Monitor>(monitor_id));
    mcp::JsonValue r(mcp::JsonValue::object_tag);
    r["result"] = mcp::JsonValue(value);
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_get_performance_monitor completed");
    return r;
}

mcp::JsonValue handle_list_performance_monitors(const mcp::JsonValue&) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_list_performance_monitors called");
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_list_performance_monitors completed");
    return r;
}

mcp::JsonValue handle_get_object_count(const mcp::JsonValue&) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_get_object_count called");
    auto* perf = godot::Performance::get_singleton();
    if (!perf) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("Performance singleton not available");
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_get_object_count completed");
    return r;
}

mcp::JsonValue handle_get_object_count_by_class(const mcp::JsonValue&) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_get_object_count_by_class called");
    mcp::JsonValue e(mcp::JsonValue::object_tag);
    e["error"] = mcp::JsonValue("object count by class not available via godot-cpp; use Performance::get_monitor(OBJECT_COUNT) for total count");
    return e;
}

mcp::JsonValue handle_get_memory_usage(const mcp::JsonValue&) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_get_memory_usage called");
    auto* perf = godot::Performance::get_singleton();
    if (!perf) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("Performance singleton not available");
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_get_memory_usage completed");
    return r;
}

mcp::JsonValue handle_profile_start(const mcp::JsonValue&) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_profile_start called");
    mcp::JsonValue e(mcp::JsonValue::object_tag);
    e["error"] = mcp::JsonValue("profiling not available via godot-cpp; use EditorInterface debug settings instead");
    return e;
}

mcp::JsonValue handle_profile_stop(const mcp::JsonValue&) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_profile_stop called");
    mcp::JsonValue e(mcp::JsonValue::object_tag);
...
mcp::JsonValue handle_profile_get_data(const mcp::JsonValue&) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_profile_get_data called");
    mcp::JsonValue e(mcp::JsonValue::object_tag);
    e["error"] = mcp::JsonValue("profiling not available via godot-cpp");
    return e;
}
...
mcp::JsonValue handle_set_fps_limit(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_set_fps_limit called");
    ...
    int64_t fps = fp->IsDouble() ? static_cast<int64_t>(fp->GetDouble()) : fp->GetInt();
    auto* engine = godot::Engine::get_singleton();
    if (!engine) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("Engine not available");
    ...
    mcp::JsonValue r(mcp::JsonValue::object_tag);
    r["result"] = mcp::JsonValue("ok");
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_set_fps_limit completed");
    return r;
}

mcp::JsonValue handle_set_physics_fps(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_set_physics_fps called");
    auto* fp = args.Find("fps");
    if (!fp || !fp->IsNumber()) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("missing required parameter: fps (integer)");
    ...
    mcp::JsonValue r(mcp::JsonValue::object_tag);
    r["result"] = mcp::JsonValue("ok");
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_set_physics_fps completed");
    return r;
}

mcp::JsonValue handle_collision_debug(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_collision_debug called");
    ...
    es->set_project_metadata("debug_options", "run_debug_collisions", godot::Variant(enabled));
    mcp::JsonValue r(mcp::JsonValue::object_tag);
    r["result"] = mcp::JsonValue("ok");
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_collision_debug completed");
    return r;
}

mcp::JsonValue handle_navigation_debug(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_navigation_debug called");
    auto* ep = args.Find("enabled");
    if (!ep || !ep->IsBool()) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("missing required parameter: enabled (bool)");
    ...
    es->set_project_metadata("debug_options", "run_debug_navigation", godot::Variant(enabled));
    mcp::JsonValue r(mcp::JsonValue::object_tag);
    r["result"] = mcp::JsonValue("ok");
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_navigation_debug completed");
    return r;
}

mcp::JsonValue handle_performance_debug(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "debug_performance_debug called");
    auto* ep = args.Find("enabled");
    if (!ep || !ep->IsBool()) {
        mcp::JsonValue e(mcp::JsonValue::object_tag);
        e["error"] = mcp::JsonValue("missing required parameter: enabled (bool)");
    ...
    es->set_project_metadata("debug_options", "run_debug_performance", godot::Variant(enabled));
    mcp::JsonValue r(mcp::JsonValue::object_tag);
    r["result"] = mcp::JsonValue("ok");
