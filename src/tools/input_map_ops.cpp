...
void persist_action(const std::string& action_name, godot::InputMap* im, godot::ProjectSettings* ps) {
        ...
        for (const char* field : {"physical_keycode", "keycode"}) {
            ...
            if (fp && fp->IsString()) {
                ...
                if (it == KEY_NAME_TO_CODE.end()) {
                    JV e(JV::object_tag);
                    e["error"] = JV("unknown key name: " + fp->GetString() + " — use numeric keycode (e.g. 65) or a known KEY_* name");
                    return e;
                }
...
JV handle_add_action(const JV& args) {
    ...
    im->add_action(godot::StringName(action.c_str()), deadzone);
    {
        auto* ps = godot::ProjectSettings::get_singleton();
        if (ps) {
            godot::Variant existing = ps->get_setting("input/" + godot::String(action.c_str()));
...
JV handle_erase_action(const JV& args) {
    ...
    im->erase_action(godot::StringName(action.c_str()));
    {
        auto* ps = godot::ProjectSettings::get_singleton();
        if (ps) {
            // ProjectSettings 无 erase 绑定，用 clear 删内存键后 save() 全量重写 project.godot
