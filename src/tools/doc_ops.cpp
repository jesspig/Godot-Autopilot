...
mcp::JsonValue handle_get_class(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_get_class called");
    ...
    result["parent_class"] = mcp::JsonValue(to_std(godot::String(cd->get_parent_class(sn))));
    result["api_type"] = mcp::JsonValue(static_cast<int64_t>(cd->class_get_api_type(sn)));
    result["can_instantiate"] = mcp::JsonValue(cd->can_instantiate(sn));
    {
        mcp::JsonValue arr(mcp::JsonValue::array_tag);
        godot::TypedArray<godot::Dictionary> methods = cd->class_get_method_list(sn, false);
    ...
    {
        mcp::JsonValue arr(mcp::JsonValue::array_tag);
        godot::TypedArray<godot::Dictionary> props = cd->class_get_property_list(sn, false);
    ...
    {
        mcp::JsonValue arr(mcp::JsonValue::array_tag);
        godot::TypedArray<godot::Dictionary> signals = cd->class_get_signal_list(sn, false);
    ...
    }
    {
        ...
        for (int i = 0; i < enum_names.size(); i++) {
            ...
            godot::PackedStringArray enum_consts = cd->class_get_enum_constants(sn, godot::StringName(ename.c_str()));
            for (int j = 0; j < enum_consts.size(); j++) {
                ...
                cv["value"] = mcp::JsonValue(static_cast<int64_t>(cd->class_get_integer_constant(sn, godot::StringName(cname.c_str()))));
    ...
    {
        mcp::JsonValue arr(mcp::JsonValue::array_tag);
        godot::PackedStringArray const_names = cd->class_get_integer_constant_list(sn, false);
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_get_class completed");
    return r;
}

mcp::JsonValue handle_search(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_search called");
    ...
    for (int i = 0; i < all_classes.size() && count < 50; i++) {
        ...
        if (name_lower.find(query_lower) != std::string::npos) {
            ...
            item["parent"] = mcp::JsonValue(to_std(godot::String(cd->get_parent_class(godot::StringName(name.c_str())))));
            item["api_type"] = mcp::JsonValue(static_cast<int64_t>(cd->class_get_api_type(godot::StringName(name.c_str()))));
    ...
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_search completed");
    return r;
}

mcp::JsonValue handle_get_method(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_get_method called");
    ...
    for (int i = 0; i < methods.size(); i++) {
        ...
        if (d.has("name")) {
            ...
            if (to_std(mname) == method_name) {
                ...
                mcp::JsonValue r(mcp::JsonValue::object_tag);
                r["result"] = std::move(result);
                LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_get_method completed");
                return r;
            }
        }
    }
    mcp::JsonValue e(mcp::JsonValue::object_tag);
    e["error"] = mcp::JsonValue("method not found: " + method_name + " in class " + class_name);
    return e;
}

mcp::JsonValue handle_get_property(const mcp::JsonValue& args) {
    LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_get_property called");
    auto* cp = args.Find("class");
    ...
    for (int i = 0; i < props.size(); i++) {
        ...
        if (d.has("name")) {
            ...
            if (to_std(pname) == prop_name) {
                ...
                mcp::JsonValue r(mcp::JsonValue::object_tag);
                r["result"] = std::move(result);
                LogSystem::instance().log(LogLevel::Info, LogCategory::Tools, "documentation_get_property completed");
                return r;
            }
        }
    }
    mcp::JsonValue e(mcp::JsonValue::object_tag);
    e["error"] = mcp::JsonValue("property not found: " + prop_name + " in class " + class_name);
    return e;
}
