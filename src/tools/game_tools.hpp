...
GDA_TOOL_CLASS(CaptureGameViewportTool, "capture_game_viewport",
               "Capture the running game's viewport as a PNG image over the runtime debug channel, returned as a base64-encoded string. Requires a game launched from the editor whose project loads the godot-autopilot extension. Optional timeout_ms bounds the capture wait (default 5000, max 30000). Optional region {x, y, width, height} crops the captured image (clamped to the image; an empty intersection fails with region_out_of_bounds) and optional max_dimension (64-4096) downscales the output so its longest side is at most that many pixels; it never upscales. With annotate=true a numbered box is drawn for every Control element found in the game scene tree and the game-side result adds annotated=true plus an elements array {id,path,type,text,position,size} in image pixels, so UI elements can be located visually and then clicked with click_game_ui_element.",
...
GDA_TOOL_CLASS_SIDE(ClickGameUiElementTool, "click_game_ui_element",
               "Click a UI element in the running game process by node path over the runtime debug channel: the game enumerates its Control tree, resolves path (exact match first, otherwise a node path ending with '/'+path) and injects a mouse_button press/release pair at the element's global_rect centre through the same path as queue_game_input. Requires a game launched from the editor whose project loads the godot-autopilot extension and the game_runtime authorization gate. path is required and comes from get_game_ui_elements; button_index (1=left default, 2=right, 3=middle) and double_click (default false, adds a second press/release pair) are optional. max_elements caps the enumeration (default 1000, max 1000); timeout_ms defaults to 5000 (max 30000). One call performs both steps (enumerate + inject) and resolves with {\"result\":{\"ok\":true,\"path\":...,\"position\":{x,y},\"clicks\":n}}; when the path is not found the error lists the element count and candidate paths.",
