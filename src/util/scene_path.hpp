...
inline std::string scene_path_hint(godot::Node* scene_root) {
    ...
    return "当前场景根为 \"" + root_name + "\"；合法路径写法："
           + root_name + "/子路径、/root/" + root_name + "/子路径、子路径";
}

inline void strip_leading_segment_if(std::string& path, const std::string& segment) {
...
inline godot::Node* resolve_scene_node(const std::string& path_str, godot::Node* scene_root,
                                       std::string* out_hint = nullptr) {
    if (!scene_root) {
        if (out_hint) *out_hint = scene_path_hint(nullptr);
        return nullptr;
    }
    ...
    godot::Node* node = scene_root->get_node_or_null(godot::NodePath(godot::String(clean.c_str())));
    if (node) {
        if (node == scene_root) {
            return node;
        }
        ...
        while (parent) {
            if (parent == scene_root) return node;
            parent = parent->get_parent();
        }
    }
    if (out_hint) *out_hint = scene_path_hint(scene_root);
    return nullptr;
}
