...
static mcp::ReadResourceResult make_json_result(const std::string& uri, const std::string& json) {
...
static const char* category_to_string(LogCategory cat) {
    switch (cat) {
        ...
        case LogCategory::Prompts: return "prompts";
        default: return "unknown";
    }
}

static std::string compute_relative_path(godot::Node* node, const std::string& root_prefix) {
...
static void node_to_json(godot::Node* node, const std::string& root_prefix, mcp::JsonValue& j) {
...
static mcp::JsonValue node_detail_to_json(godot::Node* node, const std::string& root_prefix) {
...
static void dir_to_json(godot::EditorFileSystemDirectory* dir, mcp::JsonValue& j) {
...
void register_all_resources(mcp::McpServer& server, CommandQueue& queue) {
    server.RegisterResource("engine-version", "godot://engine/version",
        mcp::ResourceOptions{}.Description("Godot engine version information").MimeType("application/json"),
        [&queue](const std::string& uri) -> mcp::ReadResourceResult {
            std::string json_str;
            queue.submit([&json_str]() {
                auto* engine = godot::Engine::get_singleton();
                if (!engine) { set_error(json_str, "Engine singleton not available"); return; }
    ...
    server.RegisterResource("scene-tree", "godot://scene/tree",
        mcp::ResourceOptions{}.Description("Current edited scene node tree").MimeType("application/json"),
        [&queue](const std::string& uri) -> mcp::ReadResourceResult {
            std::string json_str;
            queue.submit([&json_str]() {
                auto* editor = godot::EditorInterface::get_singleton();
                if (!editor) { set_error(json_str, "Not in editor mode"); return; }
                auto* root = editor->get_edited_scene_root();
                if (!root) { set_error(json_str, "No scene open"); return; }
    ...
    server.RegisterResourceTemplate("scene-node", "godot://scene/{path}",
        mcp::ResourceOptions{}.Description("Get properties of a scene node by path").MimeType("application/json"),
        [&queue](const std::string& uri, const std::map<std::string, std::string>& vars) -> mcp::ReadResourceResult {
            std::string path = vars.at("path");
            std::string json_str;
            queue.submit([&json_str, &path]() {
                auto* editor = godot::EditorInterface::get_singleton();
                if (!editor) { set_error(json_str, "Not in editor mode"); return; }
                auto* root = editor->get_edited_scene_root();
                if (!root) { set_error(json_str, "No scene open"); return; }
                ...
                if (!node) { set_error(json_str, "Node not found: " + path); return; }
    ...
    server.RegisterResource("filesystem-tree", "godot://filesystem/tree",
        mcp::ResourceOptions{}.Description("Project file system structure").MimeType("application/json"),
        [&queue](const std::string& uri) -> mcp::ReadResourceResult {
            std::string json_str;
            queue.submit([&json_str]() {
                auto* editor = godot::EditorInterface::get_singleton();
                ...
                if (!fs) { set_error(json_str, "File system not available"); return; }
                auto* root_dir = fs->get_filesystem();
                if (!root_dir) { set_error(json_str, "File system still scanning"); return; }
    ...
    server.RegisterResourceTemplate("filesystem-path", "godot://filesystem/{path}",
        mcp::ResourceOptions{}.Description("Get file info or directory listing by path").MimeType("application/json"),
        [&queue](const std::string& uri, const std::map<std::string, std::string>& vars) -> mcp::ReadResourceResult {
            std::string path = vars.at("path");
            std::string json_str;
            queue.submit([&json_str, &path]() {
    ...
    server.RegisterResource("editor-selection", "godot://editor/selection",
        mcp::ResourceOptions{}.Description("Currently selected nodes in the editor").MimeType("application/json"),
        [&queue](const std::string& uri) -> mcp::ReadResourceResult {
            std::string json_str;
            queue.submit([&json_str]() {
                auto* editor = godot::EditorInterface::get_singleton();
                ...
                if (!sel) { json_str = "[]"; return; }

                auto nodes = sel->get_selected_nodes();
                auto* root = editor->get_edited_scene_root();
                std::string root_prefix;
                if (root) root_prefix = to_std(root->get_path());
                ...
                for (int i = 0; i < nodes.size(); i++) {
                    ...
                    item["path"] = mcp::JsonValue(compute_relative_path(node, root_prefix));
    ...
    server.RegisterResourceTemplate("editor-setting", "godot://editor/settings/{key}",
        mcp::ResourceOptions{}.Description("Get an editor setting value by key").MimeType("application/json"),
        [&queue](const std::string& uri, const std::map<std::string, std::string>& vars) -> mcp::ReadResourceResult {
            std::string key = vars.at("key");
            std::string json_str;
            queue.submit([&json_str, &key]() {
                auto* editor = godot::EditorInterface::get_singleton();
                if (!editor) { set_error(json_str, "Not in editor mode"); return; }
                auto settings = editor->get_editor_settings();
                if (settings.is_null()) { set_error(json_str, "Editor settings not available"); return; }
    ...
    server.RegisterResource("log-recent", "godot://log/recent",
        mcp::ResourceOptions{}.Description("Recent log entries from the plugin's log system").MimeType("application/json"),
        [&queue](const std::string& uri) -> mcp::ReadResourceResult {
            std::string json_str;
            queue.submit([&json_str]() {
                ...
                for (size_t i = start; i < count; i++) {
                    ...
                    item["category"] = mcp::JsonValue(category_to_string(entry->category));
