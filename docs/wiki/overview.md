...
- 工具总数：README.md / README_zh.md 仍为 "~384 MCP Tools across 27 categories"（`README.md:22` / `README_zh.md:22`，约数口径，待后续版本同步），与 wiki 实测 **可达 392 / catalog 393 / 27 类 = 385**（以工具 category 字段统计为准，Capture 1→2）基本一致；`ToolRegistry`/catalog 口径自洽（registry 393 = 385 域 + `system_status` + 7 元）✓
