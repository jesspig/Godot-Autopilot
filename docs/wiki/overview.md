...
> 事实来源：根 `README.md` / `README.en.md` / `AGENTS.md`、`CMakeLists.txt`、`cmake/FetchDependencies.cmake`、`src/main.cpp`、`src/core/server_context.cpp`、`src/tools/tool_registry.hpp`、`src/tools/*_tools.hpp`、`src/tools/dispatch.cpp`、`src/prompts/prompt_handlers.cpp`、`src/resources/resource_handlers.cpp`、`Example/project.godot`、`Example/docs/`。
...
### MCP 工具：可达 392 个 = 7 元工具 + 385 领域工具

数值以 `src/tools/tool_registry.hpp`（`ToolRegistry` 单一来源）与 `src/tools/*_tools.hpp`（385 域工具，`make_tools()` 提供）为准，catalog/index 393 条目由 `register_all.cpp` 从 registry 派生；`get_active_registry()` 暴露活跃 registry，数量随插件版本变化，运行时可经 `search_tools` 确认。工具名遵循 `<动词>_<类别>_<维度>_<对象>_<修饰>`（动词置首，snake_case）。
...
**385 个领域工具**（经 `call_tool` 代理，`g_handlers` 映射分发），按 27 个类别组织（InputMap 并入 Input；TileMap 含 tileset），各类别数量以工具 category 字段/catalog 统计为准（如 `get_debug_object_info` 定义于 `physics_tools.hpp` 但归类 Debug）：
...
| Scene | 15（scene 7+scene_tree 8） | SpriteFrames | 3 |
| Config | 13 | Analysis | 3 |
| Testing | 2 | System | 1 |
| Capture | 2 |  |  |

另有 `system_status`（端口/版本/运行时长，FnTool）经 `call_tool` 调用，不直接注册为 MCP 工具；`ToolCatalog`/BM25 index 合计 393 条目（385 域 + `system_status` + 7 元工具）。
...
| 绑定层 | godot-cpp，GIT_TAG `10.0.0-rc2` | `cmake/FetchDependencies.cmake` |
...
│   │                           #   ResourceRegistry、SceneDirtyTracker、ExportGuard、ServerContext、PluginConfig、EditorReadiness、ErrorWatermark、EditorCoords
│   ├── tools/                  # 领域工具：50 个 .cpp（45 个 *_ops 含 8 个 schema_* + register_all/dispatch/...）、
│   │                           #   30 个域 _tools.hpp（385 条，另有 meta_tools.hpp 0 条承载 7 元）+ tool_registry.hpp
...
├── tests/                      # L1 gda_unit_tests（243 个 gtest，含 game_ui_coords 6 / keycode_alias 6 / script_freshness 7 / uid_guard 2 用例）+ L2 gda_test_runner + config/*.json（25 份）
...
| 工具总数 | MCP 可达 392 = 7 元 + 385 领域（领域 27 类别，其中 Capture 2）；ToolCatalog/index 393 条目（含 `system_status`） |
| 提示词 / 资源 | 7 模板 / 15 资源 |
...
- 工具总数：`README.md` / `README.en.md` 口径为 badge `tools-385+`（`README.md:8` / `README.en.md:8`）与“385+ 个工具 / Over 385 tools”（`README.md:26` / `README.en.md:26`，域工具口径，数量随版本增长以 `search_tools` 查到的为准），与 wiki 实测 **可达 392 / catalog 393 / 27 类 = 385**（以工具 category 字段统计为准）一致；`ToolRegistry`/catalog 口径自洽（registry 393 = 385 域 + `system_status` + 7 元）✓
