...
### MCP 工具：可达 391 个 = 7 元工具 + 384 领域工具

数值以 `src/tools/tool_registry.hpp`（`ToolRegistry` 单一来源）与 `src/tools/*_tools.hpp`（384 域工具，`make_tools()` 提供）为准，catalog/index 392 条目由 `register_all.cpp` 从 registry 派生；`get_active_registry()` 暴露活跃 registry，数量随插件版本变化，运行时可经 `search_tools` 确认。工具名遵循 `<动词>_<类别>_<维度>_<对象>_<修饰>`（动词置首，snake_case）。
...
**384 个领域工具**（经 `call_tool` 代理，`g_handlers` 映射分发），按 27 个类别组织（InputMap 并入 Input；TileMap 含 tileset），各类别数量以工具 category 字段/catalog 统计为准（如 `get_debug_object_info` 定义于 `physics_tools.hpp` 但归类 Debug）：
...
另有 `system_status`（端口/版本/运行时长，FnTool）经 `call_tool` 调用，不直接注册为 MCP 工具；`ToolCatalog`/BM25 index 合计 392 条目（384 域 + `system_status` + 7 元工具）。
...
│   │                           #   30 个域 _tools.hpp（384 条，另有 meta_tools.hpp 0 条承载 7 元）+ tool_registry.hpp
...
├── tests/                      # L1 gda_unit_tests（238 个 gtest）+ L2 gda_test_runner + config/*.json（17 份）
...
| 工具总数 | MCP 可达 391 = 7 元 + 384 领域（领域 27 类别）；ToolCatalog/index 392 条目（含 `system_status`） |
| 提示词 / 资源 | 7 模板 / 15 资源 |
...
- 工具总数：README.md / README_zh.md 已同步为 "~384 MCP Tools across 27 categories"（`README.md:22` / `README_zh.md:22`，注明 count varies by plugin version），与 wiki 实测 **可达 391 / catalog 392 / 27 类 = 384**（以工具 category 字段统计为准）一致；`ToolRegistry`/catalog 口径自洽（registry 392 = 384 域 + `system_status` + 7 元）✓
