...
> 覆盖范围：`tests/` 全部（unit 33 文件、runner 7 实现 + 6 头文件、integration、config 30 JSON、`tests/guard/` 迁移守卫与注释守卫、`tests/CMakeLists.txt`），对照 `tests/README.md` 与仓库根 `AGENTS.md` 测试段逐条核算。312 项 L1 为 `TEST`/`TEST_F` 宏逐行统计口径（09-21 可重放监控与日志系统批次 285→312：新增 `monitor_test` 9 / `perf_sampler_test` 6，`trace_recorder_test` 11→18、`log_system_test` 7→9、`log_persist_test` 5→8，unit 文件 31→33；09-20 可重放监控批次 269→285：新增 `trace_recorder_test` 11 / `log_persist_test` 5，unit 文件 29→31；09-19 数据化重构 243→255：新增 `tool_args_test` 11 项、`tool_registry_test` 5→6；收尾第二轮 255→269：新增 `tool_pipeline_traits_test` 4 / `tool_invoke_test` 4 / `batch_refs_test` 6；历史 09-17 精简批次 267→243：editor_coords 37→15、vision_assist 8→7、tool_catalog 删 `PopulateDefaultToolsOnce`；09-16 历史：新增 vision_assist 8 项 + 4 文件 21 项、client_config_gen 11→20）；ctest 注册点 344 = 312 gtest + 2 守卫 + 30 份 L2（L2 与守卫计数可静态核实，最终以 `ctest -N` 为准）。
...
> 09-21 可重放监控与日志系统批次：新增 L1 `monitor_test` 9 项 + `perf_sampler_test` 6 项，`trace_recorder_test` 11→18、`log_system_test` 7→9、`log_persist_test` 5→8（285→312 gtest，unit 文件 31→33），L2 新增 `30_observability`（29→30 份，3 stage：`system_status` 断言 `tool_count`/`queue_depth`；扫描 traces jsonl 断言 `kind=lifecycle`/`snapshot`/`protocol_request` 与新字段；扫描 logs 断言含 `trace=` 的 detail 行）；`ctest -E "^gda_runner_"` 287→314、全量 ctest 注册点 316→344；L2 会话实测观测到 jsonl 事件种类 `lifecycle`/`protocol_request`/`protocol_response`/`tool_call`/`persist_health`/`perf`/`snapshot`/`concurrency`，`tools/call` 的 trace_id/span 与 `call_tool`、域工具 span 正确嵌套、`request_id` 贯通、`protocol_response` 回填 `duration_ms`。
...
| `log_system_test.cpp` | 9 | 单例、级别/分类/文本过滤、环形缓冲覆盖、值拷贝快照与 recent 上限（09-02 增补）；09-21 增补 `trace_id`/`span_id` 关联字段与 `filter_text` 匹配 detail/`trace_id`/`span_id` 用例 |
...
| `trace_recorder_test.cpp` | 18 | `TraceRecorder` 纯逻辑（09-20 可重放监控批次新增）：id 前缀/唯一性、seq 严格递增、`query_recent` 尾取与 0/超量边界、`query_by_trace` 过滤、`query_since` 边界与 `next_seq`、`clear_for_test` 重置、脱敏剥离敏感键值保留其他键、脱敏/原始两种截断口径、FNV-1a 稳定值与区分、base64 解码（padding 省略/噪声字符）、`to_json_line` 核心键与转义；09-21 增补 `TraceKind`/`trace_kind_name`、schema v2 新字段与敏感键通用集合、`query_by_request` 用例 |
| `log_persist_test.cpp` | 8 | `LogPersist` 纯函数（09-20 可重放监控批次新增）：`format_human_line` 三态（含 detail/不含/空 detail）、`session_stamp_from_ticks` UTC 转换、`should_prune` 边界（>20 文件或 >50MB）、`level_name`/`category_name` 全枚举与未知值；09-21 增补健康计数访问器与 `health_summary`、`trace_dir` 用例 |
| `monitor_test.cpp` | 9 | 统一埋点门面 `monitor` 纯逻辑（09-21 可重放监控与日志系统批次新增）：`trace_kind_name` 全枚举、`RequestRegistry` 的 put/get/take/size/clear 与 `expire` 边界、`emit` 严格递增 seq 与默认填充（`thread_id`/`monotonic_ns`）、`tool_call` 失败态与人类日志按 trace_id 可检索、`begin_request`/`end_request` 经 traceparent 关联与 `query_by_request`、`push_request`/`pop_request`/`RequestScope`、`build_attrs`/`json_escape` 转义、`sanitize_field` 截断与大小写不敏感剥离 |
| `perf_sampler_test.cpp` | 6 | 性能采样纯函数（09-21 可重放监控与日志系统批次新增，仅 header 内联部分）：`accumulate_frame`（负 delta 归零与峰值跟踪）、`average_frame_ms`（空样本为 0）、`compute_rates`（`seconds<=0` 全 0 与手算窗口的吞吐/错误率/平均延迟）、零响应/零样本时比率归 0、`milliseconds_to_nanoseconds` 换算；`tick`/`set_queue`/超时看门狗依赖引擎与 `perf_sampler.cpp`，L1 未覆盖 |
| **合计** | **312** | |

**链接来源**（`tests/CMakeLists.txt` 的 `GDA_UNIT_BUSINESS_SOURCES`，共 26 个显式 + 1 组 glob）：`src/core/` 的 `log_system`、`log_persist`、`resource_registry`、`scene_dirty_tracker`、`export_guard`、`editor_readiness`、`editor_coords`、`trace_recorder`、`monitor`；`src/util/` 的 `bm25_index`、`error_util`、`readback_util`、`scene_verify`、`variant_json`、`client_config_gen`、`skill_gen`、`skill_content_generated`（构建期嵌入薄胶水）；`src/tools/` 的 `tool_catalog`、`schema_builder`、`register_all`、`dispatch`、`debugger_access`、`editor_ui_actions`、`tool_args`、`tool_pipeline`、`tool_invoke`；`src/tools/*_ops.cpp`（`GDA_TOOLS_OPS_SOURCES` glob，`register_all.cpp` 引用全部 `handle_xxx` 符号故必须链接）。

**约束**：L1 禁止调用任何已注册工具 handler（无引擎时 godot-cpp 接口指针为 nullptr 会崩溃）；`register_all_test` 仅测注册/分发/未知工具错误路径。`SchemaStatisticsBaseline`（`register_all_test.cpp:131-148`）为运行时统计：当前基线断言 catalog 399 总条 = 336 非空 + 63 空（09-19 复核；随工具集变化，以运行时统计为准）。`client_config_gen` 为纯 C++（仅 std + `mcp::JsonValue`），不触碰 Godot API，可安全纳入 L1。`sanitize_policy.cpp` 未列入 L1 业务源——`enabled()`/`set_enabled()` 为 header 内联原子缓存（L1 经 `TraceRecorder::sanitize_args(text, bool)` 直接传参断言脱敏口径），`initialize()` 仅由入口调用、不参与 L1 链接。09-21 新增 `src/core/monitor.cpp` 显式列入 L1 业务源（`monitor_test` 覆盖）；`monitor_env.cpp` 仅主线程调用、`perf_sampler.cpp` 依赖引擎 tick，二者不进 L1。
...
**30 个用例文件 → 30 条 ctest 用例**（`tests/CMakeLists.txt` 的 config GLOB 注册段：`gda_runner_<文件名去后缀>`，`TIMEOUT 600`；21 号段空缺，99 号诊断用例已删；数量以 `tests/config/*.json` 实测为准）：
...
| `gda_runner_30_observability` | `30_observability.json` | observability | 3 | 可重放监控与日志系统端到端（09-21 新增，headless）：`system_status` 断言 `tool_count`/`queue_depth` → `execute_script` 扫描 `user://godot_autopilot/traces` 最新 `trace-*.jsonl`（断言含 `kind=lifecycle`/`snapshot`/`protocol_request` 与 schema v2 新字段）→ 扫描 `user://godot_autopilot/logs` 最新 `gda-*.log`（断言含 `trace=` 的 detail 行）；**需 `code_execute` 授权**（env 优先） |
...
|---|---|---|---|
| L1 gtest 数量 | 312（33 个 unit 文件；09-21 新增 `monitor_test` 9 / `perf_sampler_test` 6，并把 `trace_recorder_test` 扩至 18、`log_system_test` 扩至 9、`log_persist_test` 扩至 8；09-20 新增 `trace_recorder_test` 11 / `log_persist_test` 5；09-19 新增 `tool_args_test` 11 项、`tool_registry_test` 5→6 增 flags 路由用例；收尾第二轮新增 `tool_pipeline_traits_test` 4 / `tool_invoke_test` 4 / `batch_refs_test` 6；历史见上方 L1 表） | 312（逐文件宏统计见上表；`ctest --preset debug -E "^gda_runner_"` = 312 + 2 守卫 = 314） | 一致 |
| L2 用例文件数 | 30（00-10 共 11 份 + 11_editor_tree / 12_capture_params / 13_inline_subresource / 14_tilemap_rect / 15_scene_path / 16_game_jobs / 17_vision_assist / 18_script_freshness / 19_cjk_roundtrip / 20_cjk_text_roundtrip / 22_uid_guard / 23_click_ui_coords / 24_keycode_alias / 25_open_scene_idempotent / 26_user_tools_code_mode / 27_user_tools_rescan / 28_sprite_frames_animation / 29_trace_persistence / 30_observability；21 号段空缺；99 号诊断用例已删） | 30（`tests/config/*.json` 实测） | 一致 |
| ctest L2 用例 | gda_runner_<name> | 一致（`tests/CMakeLists.txt` 的 config GLOB 注册段，TIMEOUT 600；30 份均由 GLOB 自动发现，新增文件零配置） | 一致 |
| ctest 注册点 | 344（312 L1 + 2 守卫 + 30 L2） | 344（其中 L2 与守卫可静态核实；最终以 `ctest -N` 实测为准） | 以运行时统计为准 |
...
- 两守卫均不随 L2 引擎环境变化；`ctest --preset debug -E "^gda_runner_"` = 312 gtest + 2 守卫 = 314，全量 ctest = 314 + 30 L2 = 344（以 `ctest -N` 为准）。
