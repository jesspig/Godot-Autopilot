...
| 层 | 载体 | 数量 | 内容 |
|---|---|---|---|
| 单一来源层 | `ToolRegistry`（`g_active_registry`） | **392** | `all()`=385（384 域 + system_status）+ `all_meta()`=7 |
| MCP 服务器层 | `server.RegisterTool()`（派生自 registry） | **7** | 元工具（见下） |
| 分发层 | `dispatch::g_handlers`+`g_meta_handlers`（派生） | **392** | 385 域/系统 + 7 元 |
| 目录层 | `ToolCatalog`（派生） | **392** | 384 域 + system_status + 7 元 |

领域工具不注册进 MCP 工具列表，只能经 `call_tool` 元工具代理调用；`ToolCatalog` 392 = 384 域工具 + `system_status` + 7 元工具（单一来源 `all_any()` 逐一 `make_tool_info` 派生）。`system_status` 是全 registry 唯一非真类来源。MCP 工具总数 = 7 元 + 384 域 = **391**（经 `call_tool` 代理可达）。
...
| 2 | 领域工具 384 个 | 30 个 `*_tools.hpp` 的 `GDA_TOOL_CLASS`/`GDA_TOOL_CLASS_SIDE` 共 384 个（324 + 60，无重复名）；registry `all()`=**385** = 384 + system_status | **一致**（system_status 是唯一非真类来源） |
| 3 | 工具总体 = 7 元 + 384 域 | registry `all_any()`=**392**（385 域/系统 + 7 元）；MCP 顶层 7 元工具，MCP 可达总数 391 | **一致** |
| 4 | ToolCatalog 392 条 | 由 registry `all_any()` 逐一 `make_tool_info` 派生 392 = 384 域 + system_status + 7 元；原 `populate_default_tools`（函数体尚存于 `tool_catalog.cpp:63` 但已无任何调用者）/meta 快照/`Auto` 补录链路整体废弃 | **一致**（单一来源派生，无独立填表） |
...
| 8 | 60 个 SIDE 宏声明工具遍历排除 | 30 个 `*_tools.hpp` 中 60 个工具用 `GDA_TOOL_CLASS_SIDE` 标记（writes_file 15 / writes_config 6 / shows_alert 4 / modifies_window 20 / process 6 / game_runtime 7 / code_execute 1 / None 1）；解析器仅枚举 `GDA_TOOL_CLASS(` 的 324 个，`side_effect` 字段兜底判定保留 | **一致** |
| 9 | 遍历步数与工具数联动 | 步数随工具数/排除集变化（域 384 中 324 个参与空参契约遍历，60 个 SIDE 不进入枚举）；**以运行时 `03_tools_contract` 统计为准** | **运行时统计口径** |
| 10 | 命名约定 `<动词>_<类别>_<维度>_<对象>_<修饰>`（动词置首） | 384 个名字全部小写 snake_case；绝大多数首段为动词（create/get/set/add/remove/apply/intersect/play/stop/save/…），5 个名词置首的历史例外（property_get/property_set/property_get_list、signal_connect/signal_disconnect） | **一致（含 5 个既有例外）** |
...
09-16 实测 `03_tools_contract` 遍历 324 个非 SIDE 工具、0 失败，warnings 恰为 `start_input_gamepad_vibration`、`stop_input_gamepad_vibration`、`get_resource_extensions`、`reimport_resource_files` 4 条（与 09-15 基线一致）。历史背景注释见 `traversal.cpp:273-275`。
...
- 清单不硬编码：60 个工具用 `GDA_TOOL_CLASS_SIDE` 标记，`get_tool_detail` 响应携带 `side_effect` 字段，`tests/runner/traversal.cpp` 仅枚举 `GDA_TOOL_CLASS(` 声明的 324 个工具并依 `side_effect` 字段兜底排除；其中空 schema 的副作用工具在冒烟阶段同样跳过。
...
> 步数随工具数变化：域工具 384（解析器仅枚举 `GDA_TOOL_CLASS(` 的 324 个，`GDA_TOOL_CLASS_SIDE` 60 个不进入枚举），空参候选 **324**，冒烟步数以运行时 `03_tools_contract.json` 统计为准。08-24 扩容后枚举与排除集同步扩大，历史实测值（546、640-660）均为旧口径，以最近一次运行为准。
...
- 384 个工具名全部匹配 `^[a-z0-9_]+$`（小写 snake_case，无大写、无连字符）；绝大多数首段为动词（create/get/set/add/remove/apply/intersect/play/stop/save/seek/move/warp/…），另有 5 个名词置首的既有例外（`property_get`/`property_set`/`property_get_list`、`signal_connect`/`signal_disconnect`）——总体符合 `<动词>_<类别>_<维度>_<对象>_<修饰>` 动词置首规范，例外为历史命名未随重构清理。
...
## 类别分布（384 领域工具，27 个类别）

> 权威总数由 30 个域 `*_tools.hpp` 的 `GDA_TOOL_CLASS`/`GDA_TOOL_CLASS_SIDE` 枚举为准（=384），registry `all()`=385（+system_status）；下表为按分类的细分（category 字段统计，2026-09-16 实测）。
...
2. **ToolCatalog 构成口径（已随本轮重构更新）**：现 392 = 384 域 + `system_status` + 7 元，全部由 registry `all_any()` 单一来源派生；原 343/332/5 默认 + 3 快照 + Auto 补录口径已废除。
3. **g_handlers 口径（已更新）**：现由 registry `all()` 派生共 385（384 域 + system_status），不再手写。
