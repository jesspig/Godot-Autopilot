...
> 审计日期：2026-09-15（2026-08-29 随 0.2.2 版本与全量审计同步；09-02 随 T0 安全边界与并发契约同步；09-13 上午随反馈修复批次同步工具数 365/372/373、Resources 26、SIDE 排除 49；09-13 下午随收口批次同步工具数 366/373/374 与 get_plugin_log；09-13 17:50 随 B 组知识库审计同步——修正契约缺口行号引用、schema 基线行号与元工具 schema 口径；09-13 晚随 0.2.4 版知识库全量审计同步——补正 populate_default_tools 存留状态与命名首段动词的 5 个既有例外；09-15 随 Computer Use grounding 批次同步工具数 379/386/387、非 SIDE 322 / SIDE 57、类别分布与遍历 warnings 实测 4 条）。
...
- **副作用驱动遍历排除**：`SideEffect` 枚举含 8 值（`None/WritesFile/WritesConfig/ShowsAlert/ModifiesWindow/Process/CodeExecute/GameRuntime`）；57 个传统副作用工具用 `GDA_TOOL_CLASS_SIDE` 宏标记（writes_file 15 / writes_config 6 / shows_alert 4 / modifies_window 19 / process 6 / game_runtime 6 / code_execute 1）；遍历 runner 仅枚举 `GDA_TOOL_CLASS(` 声明的 322 个工具，其余经 `side_effect` 字段兜底判定排除。
...
| 层 | 载体 | 数量 | 内容 |
|---|---|---|---|
| 单一来源层 | `ToolRegistry`（`g_active_registry`） | **387** | `all()`=380（379 域 + system_status）+ `all_meta()`=7 |
| MCP 服务器层 | `server.RegisterTool()`（派生自 registry） | **7** | 元工具（见下） |
| 分发层 | `dispatch::g_handlers`+`g_meta_handlers`（派生） | **387** | 380 域/系统 + 7 元 |
| 目录层 | `ToolCatalog`（派生） | **387** | 379 域 + system_status + 7 元 |

领域工具不注册进 MCP 工具列表，只能经 `call_tool` 元工具代理调用；`ToolCatalog` 387 = 379 域工具 + `system_status` + 7 元工具（单一来源 `all_any()` 逐一 `make_tool_info` 派生）。`system_status` 是全 registry 唯一非真类来源。MCP 工具总数 = 7 元 + 379 域 = **386**（经 `call_tool` 代理可达）。
...
| 2 | 领域工具 379 个 | 30 个 `*_tools.hpp` 的 `GDA_TOOL_CLASS`/`GDA_TOOL_CLASS_SIDE` 共 379 个（322 + 57，无重复名）；registry `all()`=**380** = 379 + system_status | **一致**（system_status 是唯一非真类来源） |
| 3 | 工具总体 = 7 元 + 379 域 | registry `all_any()`=**387**（380 域/系统 + 7 元）；MCP 顶层 7 元工具，MCP 可达总数 386 | **一致** |
| 4 | ToolCatalog 387 条 | 由 registry `all_any()` 逐一 `make_tool_info` 派生 387 = 379 域 + system_status + 7 元；原 `populate_default_tools`（函数体尚存于 `tool_catalog.cpp:63` 但已无任何调用者）/meta 快照/`Auto` 补录链路整体废弃 | **一致**（单一来源派生，无独立填表） |
...
| 8 | 57 个副作用工具遍历排除 | 30 个 `*_tools.hpp` 中 57 个工具用 `GDA_TOOL_CLASS_SIDE` 标记（writes_file 15 / writes_config 6 / shows_alert 4 / modifies_window 19 / process 6 / game_runtime 6 / code_execute 1）；解析器仅枚举 `GDA_TOOL_CLASS(` 的 322 个，`side_effect` 字段兜底判定保留 | **一致** |
| 9 | 遍历步数与工具数联动 | 步数随工具数/排除集变化（域 379 中 322 个参与空参契约遍历，57 个 SIDE 不进入枚举）；**以运行时 `03_tools_contract` 统计为准** | **运行时统计口径** |
| 10 | 命名约定 `<动词>_<类别>_<维度>_<对象>_<修饰>`（动词置首） | 379 个名字全部小写 snake_case；绝大多数首段为动词（create/get/set/add/remove/apply/intersect/play/stop/save/…），5 个名词置首的历史例外（property_get/property_set/property_get_list、signal_connect/signal_disconnect） | **一致（含 5 个既有例外）** |
...
09-15 实测 `03_tools_contract` 遍历 322 个非 SIDE 工具、0 失败，warnings 恰为 `start_input_gamepad_vibration`、`stop_input_gamepad_vibration`、`get_resource_extensions`、`reimport_resource_files` 4 条。历史背景注释见 `traversal.cpp:273-275`。
...
- **23 个用户可见副作用**：`shows_alert` 4 个（show_os_alert、show_display_dialog、speak_display_tts、stop_display_tts）+ `modifies_window` 19 个（原 12 个：set_display_clipboard、set_display_mouse_mode、warp_display_mouse、set_display_window_title、set_display_window_position、set_display_window_size、set_display_window_mode、set_display_window_flag、move_display_window_to_foreground、request_display_window_attention、create_display_window、delete_display_window；09-15 新增 7 个：click_editor_element、type_editor_element_text、run_editor_shortcut、click_input_mouse、scroll_input_mouse、drag_input_mouse、type_input_text）。
...
- **6 个游戏运行时副作用**（`game_runtime`）：execute_game_script、reload_game_scripts、queue_game_input、wait_game_input、sequence_game_inputs、click_game_ui_element（09-15 新增）。
- **1 个任意脚本副作用**（`code_execute`）：execute_script。
- 清单不硬编码：57 个工具用 `GDA_TOOL_CLASS_SIDE` 标记，`get_tool_detail` 响应携带 `side_effect` 字段，`tests/runner/traversal.cpp` 仅枚举 `GDA_TOOL_CLASS(` 声明的 322 个工具并依 `side_effect` 字段兜底排除；其中空 schema 的副作用工具在冒烟阶段同样跳过。
...
> 步数随工具数变化：域工具 379（解析器仅枚举 `GDA_TOOL_CLASS(` 的 322 个，`GDA_TOOL_CLASS_SIDE` 57 个不进入枚举），空参候选 **322**，冒烟步数以运行时 `03_tools_contract.json` 统计为准。08-24 扩容后枚举与排除集同步扩大，历史实测值（546、640-660）均为旧口径，以最近一次运行为准。
...
- 379 个工具名全部匹配 `^[a-z0-9_]+$`（小写 snake_case，无大写、无连字符）；绝大多数首段为动词（create/get/set/add/remove/apply/intersect/play/stop/save/seek/move/warp/…），另有 5 个名词置首的既有例外（`property_get`/`property_set`/`property_get_list`、`signal_connect`/`signal_disconnect`）——总体符合 `<动词>_<类别>_<维度>_<对象>_<修饰>` 动词置首规范，例外为历史命名未随重构清理。
...
## 类别分布（379 领域工具，27 个类别）

> 权威总数由 30 个域 `*_tools.hpp` 的 `GDA_TOOL_CLASS`/`GDA_TOOL_CLASS_SIDE` 枚举为准（=379），registry `all()`=380（+system_status）；下表为按分类的细分。
