...
- **埋点 lifecycle（09-21 新增）**：`register_all_tools`/`refresh_derived`/`refresh_dynamic_tools`/`server.RegisterTool` 批量注册处分别发 `monitor::lifecycle`（`registry_build`/`registry_refresh_derived`/`registry_refresh_dynamic`/`tools_registered`），随 trace jsonl 落盘以还原注册管线时序。
...
- 元工具处理器（09-21 起）用 `RequestContext::GetRequest().id` 建立 `monitor::RequestScope` 并经 `tools::RequestSpanGuard` 压入请求 span，使「协议请求 → call_tool → 域工具」trace 端到端关联（`request_id` 贯通）。
- `system_status`（`SpecTool`，非元工具，经 `call_tool` 调用）09-21 起返回新增 `tool_count`（工具总数）与 `queue_depth`（`CommandQueue::stats().pending`）字段；工具总数仍 399，未新增工具。
...
- `call_handler`：编辑器队列存在且不在主线程时，经 `CommandQueue::execute_sync` 排到主线程执行；handler map 在锁内替换、复制后锁外执行。09-20 起在提交前 `tools::capture_trace_context()`、主线程 lambda 内用 `ScopedTraceContext` 恢复调用方 trace 上下文并按 `steady_clock` 计算排队等待（`set_queued_wait_ms`，由 `SpecTool::execute` 一次性取走写入 `queue_wait_ms`）；09-21 起 `call_handler` 捕获/恢复 `request_id`（`monitor::RequestScope`），handlers 互斥锁用 `LockProbe`（阈值 5ms，超阈值才记录），`record_dispatch_exception` 经 `monitor::tool_call` 补 `error_type`。全仓共 22 处 `ScopedTraceContext` 恢复点（与 [ToolSpec 数据层与执行管线](../tool_base_design.md)同口径）：`resource_handlers.cpp` 8、`debugger_resources.cpp` 7、`autopilot_tools.cpp` 3、`code_exec_ops.cpp` 2、`register_all.cpp` 1、`dispatch.cpp` 1；`tool_invoke.hpp/cpp` 的 7 处为类型声明与构造/析构实现、不计入。
