...
- **副作用驱动遍历排除**：`SideEffect` 枚举含 8 值（`None/WritesFile/WritesConfig/ShowsAlert/ModifiesWindow/Process/CodeExecute/GameRuntime`）；62 个工具用 `GDA_TOOL_CLASS_SIDE` 宏标记（writes_file 16 / writes_config 6 / shows_alert 4 / modifies_window 20 / process 6 / game_runtime 7 / code_execute 1 / None 2——`fill_tilemap_rect` 与 `build_nodes_from_spec` 以 SIDE 宏声明但 side_effect 为 None，仅靠宏前缀排除）；遍历 runner 仅枚举 `GDA_TOOL_CLASS(` 声明的 329 个工具，其余经 `side_effect` 字段兜底判定排除。
...
领域工具不注册进 MCP 工具列表，只能经 `call_tool` 元工具代理调用；`ToolCatalog` 399 = 391 域工具 + `system_status` + 7 元工具（单一来源 `all_any()` 逐一 `make_tool_info` 派生）。`system_status` 是全 registry 唯一非真类来源。MCP 工具总数 = 7 元 + 391 域 = **398**（经 `call_tool` 代理可达）。
...
| 2 | 领域工具 391 个 | 30 个 `*_tools.hpp` 的 `GDA_TOOL_CLASS`/`GDA_TOOL_CLASS_SIDE` 共 391 个（329 + 62，无重复名）；registry `all()`=**392** = 391 + system_status | **一致**（system_status 是唯一非真类来源） |
| 3 | 工具总体 = 7 元 + 391 域 | registry `all_any()`=**399**（392 域/系统 + 7 元）；MCP 顶层 7 元工具，MCP 可达总数 398 | **一致** |
| 4 | ToolCatalog 399 条 | 由 registry `all_any()` 逐一 `make_tool_info` 派生 399 = 391 域 + system_status + 7 元；原 `populate_default_tools`/meta 快照/`Auto` 补录链路已随 09-17 精简批次整体删除——catalog 无默认填充，完全由 ToolRegistry 经 `replace_tools` 动态填充（`register_all.cpp:465`） | **一致**（单一来源派生，无独立填表） |
...
| 8 | 62 个 SIDE 宏声明工具遍历排除 | 30 个 `*_tools.hpp` 中 62 个工具用 `GDA_TOOL_CLASS_SIDE` 标记（writes_file 16 / writes_config 6 / shows_alert 4 / modifies_window 20 / process 6 / game_runtime 7 / code_execute 1 / None 2）；解析器仅枚举 `GDA_TOOL_CLASS(` 的 329 个，`side_effect` 字段兜底判定保留 | **一致** |
| 9 | 遍历步数与工具数联动 | 步数随工具数/排除集变化（域 391 中 329 个参与空参契约遍历，62 个 SIDE 不进入枚举）；**以运行时 `03_tools_contract` 统计为准** | **运行时统计口径** |
| 10 | 命名约定 `<动词>_<类别>_<维度>_<对象>_<修饰>`（动词置首） | 391 个名字全部小写 snake_case；绝大多数首段为动词（create/get/set/add/remove/apply/intersect/play/stop/save/…），5 个名词置首的历史例外（property_get/property_set/property_get_list、signal_connect/signal_disconnect） | **一致（含 5 个既有例外）** |
...
- 清单不硬编码：62 个工具用 `GDA_TOOL_CLASS_SIDE` 标记，`get_tool_detail` 响应携带 `side_effect` 字段，`tests/runner/traversal.cpp` 仅枚举 `GDA_TOOL_CLASS(` 声明的 329 个工具并依 `side_effect` 字段兜底排除；其中空 schema 的副作用工具在冒烟阶段同样跳过。
...
> 步数随工具数变化：域工具 391（解析器仅枚举 `GDA_TOOL_CLASS(` 的 329 个，`GDA_TOOL_CLASS_SIDE` 62 个不进入枚举），空参候选 **329**，冒烟步数以运行时 `03_tools_contract.json` 统计为准。08-24 扩容后枚举与排除集同步扩大，历史实测值（546、640-660）均为旧口径，以最近一次运行为准。
...
- 391 个工具名全部匹配 `^[a-z0-9_]+$`（小写 snake_case，无大写、无连字符）；绝大多数首段为动词（create/get/set/add/remove/apply/intersect/play/stop/save/seek/move/warp/…），另有 5 个名词置首的既有例外（`property_get`/`property_set`/`property_get_list`、`signal_connect`/`signal_disconnect`）——总体符合 `<动词>_<类别>_<维度>_<对象>_<修饰>` 动词置首规范，例外为历史命名未随重构清理。
...
> 权威总数由 30 个域 `*_tools.hpp` 的 `GDA_TOOL_CLASS`/`GDA_TOOL_CLASS_SIDE` 枚举为准（=391），registry `all()`=392（+system_status）；下表为按分类的细分（category 字段统计，2026-09-16 实测）。
