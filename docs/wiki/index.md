...
| [tool_base_design.md](tool_base_design.md) | ToolBase 工具统一标准化（接口 + 组合 + 真类化）：379 域工具全为独立 ToolBase 子类、ToolRegistry 唯一来源、tool_defs.def 删除 | `tool_base.hpp`、`tool_registry.hpp`、`fn_tool.hpp`、`tool_decl.hpp`、30 个域 `*_tools.hpp`、`register_all.cpp` |
...
| [modules/tools_ops_a.md](modules/tools_ops_a.md) | 领域工具 A 组（场景/属性/输入/物理/导航/资源/脚本/配置/文档/编辑器 16 模块，191 工具） | `src/tools/*_ops.cpp` |
| [modules/tools_ops_b.md](modules/tools_ops_b.md) | 领域工具 B 组（调试/显示/OS/运行时/音频/渲染/瓦片等 18 模块，188 工具；A 191 + B 188 = 379） | `src/tools/*_ops.cpp` |
...
- 工具注册总入口 `ToolRegistry`（单一来源）：**387 条目** = 379 域工具 + `system_status` + 7 元工具；域工具分 **27 类**（InputMap 并入 Input）；MCP 可达工具总数 **386** = 7 元 + 379 域
- schema：非空/空数以运行时统计为准；**3 个契约缺口**（create_scene_node、get_resource_extensions、reimport_resource_files）
- 遍历排除 **57 个副作用工具**（域 379 个中解析器仅按 `GDA_TOOL_CLASS(` 枚举 322 个，`GDA_TOOL_CLASS_SIDE` 57 个不进入枚举，side_effect 字段兜底判定保留）；L1 单元测试 **172 个 gtest**；L2 引擎用例 **11 个文件**（00_meta/01_scene/02_property/03_tools_contract/04_resources_scripts/05_rename_references/06_move_references/07_scene_tabs/08_property_readback/09_editor_ui/10_editor_input）；当前 ctest 注册点为 **183**（172 L1 + 11 L2，L2 需引擎环境与 `GODOT_AUTOPILOT_ALLOW`）
