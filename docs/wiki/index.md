...
- 遍历排除 **62 个 `side_effect`/`mutating` 工具**（运行时经 `search_tools` 空 query 枚举 392 条 = 391 域 + `system_status`，跳过 7 元后按 `side_effect`/`mutating`/`dynamic` 字段排除，330 个进入空参+冒烟两步骤；`dynamic` 分支为防回归保留）；L1 单元测试 **269 个 gtest**（29 个 unit 文件）+ 迁移守卫与注释守卫各 1 项（`ctest -E "^gda_runner_"` 共 271）；L2 引擎用例 **28 个文件**（00_meta-10_editor_input 11 份 + 11_editor_tree/12_capture_params/13_inline_subresource/14_tilemap_rect/15_scene_path/16_game_jobs/17_vision_assist/18_script_freshness/19_cjk_roundtrip/20_cjk_text_roundtrip/22_uid_guard/23_click_ui_coords/24_keycode_alias/25_open_scene_idempotent/26_user_tools_code_mode/27_user_tools_rescan/28_sprite_frames_animation；21 号段空缺，99 号诊断用例已删）；ctest 注册点 **299**（269 + 2 守卫 + 28；以 `ctest -N` 为准，L2 需引擎环境与 `GODOT_AUTOPILOT_ALLOW`，26/27 号另需 `user_tools`）
