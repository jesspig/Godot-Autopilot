...
tags:
  ...
  - 并发
  - 契约
timestamp: "2026-09-08T01:57:10+08:00"
resource:
...
| 任意脚本 | `code_execute` | 即使没有 `side_effect` 标记，也按最高风险处理；脚本可读写场景、资源、文件并调用方法。 |
...
- MCP 工具面之外存在一处工程内写盘途径：编辑器面板 "Generate Skills" 按钮（`skill_gen`），固定写入 `<res://>/.agents/skills/`，仅覆盖 godot-autopilot 自有 19 册命名空间、不触碰其他文件；该途径不经 MCP 暴露、无远程触发面。
