...
- **工具命名**：约定 `<动词>_<类别>_<维度>_<对象>_<修饰>`（snake_case，**动词置首**），如 `create_scene_node`、`intersect_physics_2d_ray`、`set_input_map_action_deadzone`；365 个工具名全部动词置首，段数随粒度自然变化（2-6 段），`signal_connect`/`property_set` 等短名属规范内省略类别段
