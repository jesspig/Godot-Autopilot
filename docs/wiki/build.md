...
- **文档一致性**：`README.md` / `README_zh.md` 工具数口径已同步为 "~379 MCP Tools across 27 categories"（`README.md:22` / `README_zh.md:22`，域工具口径、自注 count varies by plugin version），与 [overview.md](./overview.md) 的 **379 域工具 / MCP 可达 386 / catalog 387** 一致——386/387 含 7 元工具与 `system_status`，与 README 域工具口径不矛盾（README 未列这两个数字属口径差异）；`--package` / `--debug` 两个 build.py 参数在 README 构建章节（`README.md:121-122`）未提及。
